...
class Shadowing:
    def __init__(self, camera: Camera, distance: float, width: int, height: int, world_width: float, light: np.ndarray, near: float = 0.1, parallel: bool = False) -> None:
        ...
        # Shadow depth buffer
        self.shadow_map = np.full((height, width), np.inf)

    def generate_shadow_map(self, objects: list[Object]) -> None:
        """Generate a depth map from the light's perspective."""
        shadow_buffer = np.full((self.height, self.width), np.inf)

        for obj in objects:
            world_vertices = obj.vertices + obj.origin
            for vertex_indices, _ in obj.planes:
                cam_vertices = [self.__world_to_camera(world_vertices[v]) for v in vertex_indices]
                clipped_vertices = self.__clip_polygon_to_near_plane(cam_vertices)

                if len(clipped_vertices) < 3:
                    continue

                for i in range(1, len(clipped_vertices) - 1):
                    projected = [self.__project_vertex(v) for v in (clipped_vertices[0], clipped_vertices[i], clipped_vertices[i + 1])]
                    self.__update_shadow_buffer(projected, shadow_buffer)

        self.shadow_map = shadow_buffer

    def __update_shadow_buffer(self, projected, shadow_buffer):
        """Update the shadow buffer with the closest depth values."""
        x1, y1, z1 = projected[0]
        x2, y2, z2 = projected[1]
        x3, y3, z3 = projected[2]

        for x in range(max(0, min(x1, x2, x3)), min(self.width - 1, max(x1, x2, x3)) + 1):
            for y in range(max(0, min(y1, y2, y3)), min(self.height - 1, max(y1, y2, y3)) + 1):
                depth = min(z1, z2, z3)
                shadow_buffer[y, x] = min(shadow_buffer[y, x], depth)

    def is_in_shadow(self, world_pos: np.ndarray) -> bool:
        """Determine if a point is in shadow."""
        light_space_pos = self.__world_to_camera(world_pos)
        px, py, pz = self.__project_vertex(light_space_pos)

        if 0 <= px < self.width and 0 <= py < self.height:
            return pz > self.shadow_map[py, px]
        return False

    def apply_shadow(self, color: list[int], world_pos: np.ndarray) -> list[int]:
        """Darken color if in shadow."""
        if self.is_in_shadow(world_pos):
            return [int(c * 0.3) for c in color]  # Darken by 70%
        return color
